...
| k_frac | 活跃通道/层 | val@12k | vs dense |
|---|---|---|---|
| 0.02 | 15 | 4.156 | +0.250 |
| 0.05 | 38 | 3.992 | +0.086 |
| 0.10 | 77 | 3.889 | −0.017 |
| 0.15 | 115 | 3.860 | −0.046 |
| **0.25** | **192** | **3.827** | **−0.079** |
| 0.40 | 307 | 3.850 | −0.056 |
| 0.50 | 384 | 3.873 | −0.033 |
| 0.60 | 461 | 3.899 | −0.007 |
| 1.00 (dense) | 768 | 3.906 | — |

**发现**：九点 U 形曲线闭合（补 50%/5% 两点），谷底在 **25% 附近**，两侧对称抬升；40–60% 仍略优于 dense，
到 100% 收敛持平。适度稀疏（每层关 3/4 通道）稳定优于 dense 约 0.08 nats，
过稀（2%）则明显受损——k-WTA 的收益是 U 形的，不是单调的。

**延长复核（24k，同一协议）**：k25 从零重训 24000 步，终点 val **3.6351**，
仍优于 std-24k（3.6731）与 10%-24k（3.6613）；领先幅度从 12k 的 0.079 收窄到 0.038
（dense 部分追上，与收敛加速模式一致），但**没有被追平**。20k→24k 三臂同步回升的
调度尾巴同样存在（k25: 3.569→3.635）。distinct3 0.734，无塌缩。

注意事项：单 seed、仅本架构；稀疏臂 tps 略低于 dense（37.5–36.7k vs 40.4k，
...
### 数据维度与规模阶梯（2026-09-20 追加）

- **48k 对**（92.6M，数据 ×4）：dense 3.546 / k25 3.525——甜点优势在 4 倍数据下保持，
  **225M 反转不是数据饥饿造成**。
- **336M 点**（d1280/L16，batch8：dense 5.266 / k25 5.191）：k25 反超 +0.075——
  216M 的反转在 336M 消失，数据饥饿区的规模效应**非单调**
  （注意：216M 对内部 batch 匹配、自洽；393M 为此前误写，实测 49M token）。
- 结论边界更新（2026-09-23 终版）：**全部匹配协议测点（四个规模、五个匹配点+0.5B 双预算档）k25 均优于 dense**，
  且 0.5B 档训练预算翻倍（token/更新/调度长度同步）后优势**翻倍**（+0.090→+0.180）——严格归因到"数据"单因子需专门控制实验，暂称训练预算缩放；且 24k 运行内部 12k 探针处 k25 落后 0.057、24k 反超——**478M 上优势是晚期涌现的 crossover**；
  优势在 0.5B 匹配对上达到最大（+0.090）；唯一反例（216M b16）系该点独有配置
  （batch16/393M token）的混淆效应。"甜点是 regime 函数"仍然成立——但反转本身
  不重复出现。
