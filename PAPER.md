...
We transplant three mechanisms of the Drosophila mushroom body into a small transformer language model and test each against pre-registered baselines. (1) Winner-take-all channel sparsification (k-WTA) accelerates convergence: at mid-training the sparse model is calibrated and accurate while the dense baseline is neither, though dense catches up given 2× the training. A sparsity sweep (7 points) shows the gain is U-shaped — keeping 25% of channels beats dense at every checkpoint we measured, while 2% clearly hurts. (2) Fly-style *memory management* — per-task parameter compartments plus a surprise gate that blocks weight updates when batch loss is below a running noise floor — reduces catastrophic forgetting by 79% in sequential domain fine-tuning, while sparse activation *per se* does not help at all. (3) A data-free "forgetting shower" (targeted decay of low-magnitude weights) removes the late-training overfitting tail, whereas the uniform-decay control destroys the model: what matters is *which* weights are forgotten. We also report a familiarity probe (set-level contamination detection AUC 0.93, single-window weak) and three negative results. We argue the productive import of the fly for language models is not sparse representation alone but the management of *when to write, what to keep, and when to erase*.
...
All arms show a synchronized val-loss rise at the end of OneCycle. From the 24k checkpoint, 500 shower steps (decay of weights in the bottom-10% magnitude band, no data, no gradients) reduce val from 3.651 to 3.588 — below the 20k low point. The control is unambiguous: uniform decay at matched strength drives val from 3.65 to 6.60 (collapse). Forgetting helps only when it forgets the right things — the ML echo of dedicated erasure circuitry in the fly.
