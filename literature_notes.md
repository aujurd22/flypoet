...
## 深读 8：性别二态分析——蘑菇体是性保守的（意外发现）

**核心发现**：1,258 个雄性特有神经元中 **0 个在蘑菇体内**。
- MB 相关（KC/MBON/DAN/olfactory）：7,140 总数，**0 雄性特有**
- 非MB感觉：198,346 总数，1,258 雄性特有（0.6%）
- **蘑菇体在性别间是结构保守的（monomorphic）**

**fru 基因交叉**：1,036/1,258（82%）雄性特有神经元为 fru_high——
这些是果蝇求偶行为回路的经典神经元（fruitless 基因是雄性求偶行为的 Master Regulator）。

**意义**：
1. **学习/记忆管理系统（MB）在性别间结构保守**——雄性和雌性用同样的"硬件"做记忆管理
2. 性别二态集中在感觉/生殖端，不在记忆端
3. **对 FlyPoet/FlyMemory 的验证**：这支持"记忆管理策略是通用的"假设——
   同一套 sparse code + write gate + compartment 机制应该跨任务/跨数据/跨域适用，
   只有输入映射需要适配

**fru 通路细节**：
| 标记 | 数量 | 含义 |
|---|---|---|
| fru_high | 2,611 | fruitless 高表达（雄性求偶回路核心） |
